...
void schedule_reboot(int delay_ms) {
  if (reboot_timer == NULL) {
    ...
    reboot_timer = xTimerCreate("reboot_timer", pdMS_TO_TICKS(delay_ms),
                                pdFALSE, (void *)0, reboot_callback);
  }
  // Start the timer
...
void schedule_ota_timeout(int timeout_ms) {
  if (ota_timeout_timer == NULL) {
    // Create a one-shot timer
    ota_timeout_timer =
        xTimerCreate("ota_timeout_timer", pdMS_TO_TICKS(timeout_ms), pdFALSE,
                     (void *)0, ota_timeout_callback);
  }
  // Start the timer
...
void ota_handler_task(void *pvParameter) {
  ...
  ESP_LOGI(TAG, "Lengths - Original: %d Payload: %d", mqtt_event->data_len,
           strlen(payload_data));
  ...
  while (attempt < max_retries) {
    ...
    EventBits_t bits =
        xEventGroupWaitBits(ota_event_group, OTA_COMPLETE_BIT | OTA_FAILED_BIT,
...
void ota_task(void *pvParameter) {
  ...
  if (esp_https_ota_is_complete_data_received(ota_handle)) {
    ...
    if (ota_finish_err == ESP_OK) {
      ...
      if (nvs_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to write OTA timestamp to NVS: %s",
